...
class RequestResult:
    ...
    def _parse_data(self):
        ...
        elif self.status_code >= 400 and not self.parse_on_error:
            data_or_error = DataParseError(reason=self.error, code=self.status_code)
...
def response_from_debug(request: RequestBuilder, result: RequestResult):
    debug_response = etree.XML(result._response._body)
    ...
    if original_response is not None:
        ...
        response = ClientResponse(result._response.method, result._response.url, writer=result._response._writer,
                                  continue100=result._response._continue, timer=result._response._timer,
                                  request_info=result._response.request_info, traces=result._response._traces,
                                  loop=result._response._loop, session=result._response._session)
