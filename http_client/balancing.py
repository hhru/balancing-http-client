...
from http_client import BalancedHttpRequest, RequestEngine, RequestEngineBuilder, RequestResult
...
class Server:
    ...
    def get_stat_load(self, current_servers):
        ...
        if not self.statistics_filled_with_initial_values:
            ...
            http_client_logger.debug(
                f'Server {self} statistics has no init value. '
...
class Upstream:
    ...
    def __init__(self, name, config_by_profile, servers):
        ...
        self.config_by_profile = (
            config_by_profile if config_by_profile else {Upstream.DEFAULT_PROFILE: self.get_default_config()}
        )
        self._update_servers(servers)
        self.allow_cross_dc_requests = (
            options.http_client_allow_cross_datacenter_requests or
            name in options.force_allow_cross_datacenter_for_upstreams
        )
...
class RequestBalancer(RequestEngine):
    ...
    async def _retryable_fetch(self) -> RequestResult:
        ...
        result.parse_response = self.request.parse_response
        result.parse_on_error = self.request.parse_on_error
        ...
        if self.request.fail_fast and result.failed:
        ...
        if self.kafka_producer is not None and not do_retry:
            ...
            asyncio.get_event_loop().create_task(
                self.kafka_producer.send(
                    'metrics_requests',
                    utf8(
                        f'{{"app":"{app}","dc":"{dc}","hostname":"{current_host}",'
