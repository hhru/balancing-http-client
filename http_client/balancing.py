...
from http_client.request_response import (FailFastError, NoAvailableServerException, ResponseData, response_from_debug,
...
class RequestBalancer(RequestEngine):
    ...
    async def execute(self) -> RequestResult:
        ...
        return await speculative_requests(request_task, speculative_request_task)
    ...
    def _unwrap_debug(self, request, result: RequestResult, retries_count):
        ...
        try:
            if result.headers.get('X-Hh-Debug'):
                debug_response = response_from_debug(request, result)
...
class UpstreamRequestBalancer(RequestBalancer):
    ...
    def _get_result_or_context(self, request: RequestBuilder):
        ...
        request.url = self.state.current_host + self.request.path
